...
Route::middleware(['auth:sanctum', 'verified'])->group(function () {
    ...
    Route::get('/', function (Request $request) {
        ...
        //ano passado no parametro
        $comprasMesAno = Ordens::where('tipoordem', 'C')->whereBetween('data', [date('Ymd', strtotime($ano . "0101")), date('Ymd', strtotime($ano . "1231"))])
        ...
        //ano passado no parametro
        $vendasMesAno = Ordens::where('tipoordem', 'V')->whereBetween('data', [date('Ymd', strtotime($ano . "0101")), date('Ymd', strtotime($ano . "1231"))])
            ->whereHas('carteira', function ($query) use ($user_id) {
                $query->where('carteiras.user_id', $user_id);
            })
            ->orderBy('data')
        ...
        $vendasMesAnoAtual = Ordens::where('tipoordem', 'V')->whereBetween('data', [date('Ymd', strtotime($anoAtual . "0101")), date('Ymd', strtotime($anoAtual . "1231"))])
            ->whereHas('carteira', function ($query) use ($user_id) {
                $query->where('carteiras.user_id', $user_id);
            })
            ->orderBy('data')
        ...
        $vendasMesAtual = 0;
        if (isset($vendasMesAnoAtual[date('m')])) {
            $vendasMesAtual = $vendasMesAnoAtual[date('m')];
        }
